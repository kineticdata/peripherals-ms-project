...
class MsprojectProjectCheckinV1
  ...
  def execute()
    ...
    checkin_endpoint = proj_resource["/_api/ProjectServer/Projects('#{@parameters['project_id']}')/Draft/checkIn()"]

    times_to_try = 6
    error_details = {:retry => true, :message => nil}
    while error_details[:retry]
      begin
        puts "Checking project '#{@parameters['project_id']}' in" if @enable_debug_logging
        checkin_endpoint.post ""
        puts "Successfully checked in the project '#{@parameters['project_id']}'"
        break
      rescue RestClient::Exception => error
